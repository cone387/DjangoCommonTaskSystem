...
if os.environ.get('USE_GUNICORN', None) == 'true':
    ...
    class ScheduleQueueModule(TextChoices):
        FIFO = "%s.%s" % (queue.Queue.__module__, queue.Queue.__name__), '先进先出队列'
        LIFO = "%s.%s" % (queue.LifoQueue.__module__, queue.LifoQueue.__name__), '后进先出队列'
        ...
        REDIS_FIFO_QUEUE = "django_common_task_system.queue.RedisFIFOQueue", 'Redis先进先出队列'
        REDIS_LIFO_QUEUE = "django_common_task_system.queue.RedisLIFOQueue", 'Redis后进先出队列'
