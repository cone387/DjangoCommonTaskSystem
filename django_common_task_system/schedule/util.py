...
from django_common_task_system.models import Schedule
...
def _get_schedule_start_end_time(schedule: Schedule, start_time=None, end_time=None):
...
def get_schedule_times(schedule: Schedule, start_time=None, end_time=None):
    schedule_times = []
...
def get_history_schedules(schedule: Schedule, start_time=None, end_time=None):
    schedules = []
    start_time, end_time = _get_schedule_start_end_time(schedule, start_time, end_time)
    schedule_config = ScheduleConfig(config=schedule.config)
    while start_time < end_time:
...
def get_missing_schedules(schedules: List[Schedule], start_time=None, end_time=None):
    result: Dict[int, List[Schedule]] = Dict[int, List[Schedule]]()
    for schedule in schedules:
        missing = result.setdefault(schedule.id, [])
        ...
        if diff % (3600 * 24) == 0:
            ...
            # 处理间隔不是整数的情况
            continue
        max_failed_times = schedule.task.config.get('max_failed_times', 3)
        ...
        command = f"""
                    ...
                    left join (
                        select schedule_time, count(status != 'F' or null) as succeed, 
                            count(status='F' or null) as failed from {ScheduleLogModel._meta.db_table} where 
                            schedule_id = {schedule.id} and 
                            schedule_time between '{start_time}' and '{end_time}'
                            group by schedule_time
                    ) b 
                    on a.date = b.schedule_time where b.succeed is null or (b.succeed = 0 and 
                    b.failed < {max_failed_times})
                """
        ...
        with connection.cursor() as cursor:
            ...
            for d, *_ in cursor.fetchall():
                ...
                o.next_schedule_time = parser.parse(d)
                missing.append(o)
    return result
