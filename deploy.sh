...
function deploy_to_server() {
  ...
  echo "docker run -d -v /var/run/docker.sock:/var/run/docker.sock $VOLUME $INIT_ENV $SET_USER_ENV $SET_PASSWORD_ENV $GUNICORN_ENV $ENV  -p $PORT:8000 --log-opt max-size=100m --name django-common-task-system $PROJECT"
